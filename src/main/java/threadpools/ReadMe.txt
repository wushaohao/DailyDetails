...
举个例子:
 ...
 1.可见性
     volatile在这里是为了保持可见性。比如thread1 和thread2 同时走到第一次判空那里，instrance 这个变量副本会拷贝到两个线程各自的工作内存里，
 然后thread1抢先拿到锁，实例化对象，释放锁。 随后thread2拿到锁，第二次判空时，由于工作内存里的instance = null, 所以仍然会实例对象。
 如果 成员变量用volatile修饰， thread2第二次判空时 会从主内存取Instance, 发现不为空，也就不会重复实例化。从而保证了对象的唯一性。
